...
OCI layout, driven by the same `Brain` / `TelegramChannel` / `ChannelBinding`
resources you would apply in a cluster (here fed through the filesystem control
plane, `AURORA_CONTROL_PLANE=fs`).
...
[`resources/`](resources) into `dist/resources` (with the layout path, sealed
token, and ids filled in), then starts the agent. Message your bot from the
configured user and it plans with the LLM and acts via the granted `k8s.*` /
`helm.*` tools. Requires Go and TinyGo 0.41.x on the host.
...
- The fs control plane resolves `resources/`, pulls the brain from
...
- The channel supervisor decrypts the Telegram token (`AURORA_SECRET_KEY`) and
  opens the bridge; the binding's flat grant scopes what the brain may do.

Generated keys live in `dist/secrets.env` so state persists across runs. Delete
`dist/` to reset. The exact same `resources/*.yaml` deploy to a cluster — see
